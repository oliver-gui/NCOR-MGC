...
def _find_conda_solver_exe(solver_name):
    """Find a solver executable in conda environment's Library/bin."""
    exe_name = solver_name + ('.exe' if sys.platform == 'win32' else '')
    search_paths = []

    # Search relative to the current Python executable (conda env)
    python_exe = Path(sys.executable)
    conda_env_root = python_exe.parent  # e.g., D:\conda_envs\torch
    conda_lib_bin = conda_env_root / 'Library' / 'bin'
    ...
    if conda_prefix:
        conda_lib_bin2 = Path(conda_prefix) / 'Library' / 'bin'
        if conda_lib_bin2.exists() and conda_lib_bin2 not in search_paths:
            search_paths.append(conda_lib_bin2)
...
def _resolve_solver_factory(solver_name, **kwargs):
    """Create a SolverFactory, auto-resolving conda env paths if needed."""
    s = SolverFactory(solver_name, **kwargs)
    if s.available():
        return s
    # Try to find in conda environment
    exe_path = _find_conda_solver_exe(solver_name)
    if exe_path:
        return SolverFactory(solver_name, executable=exe_path, **kwargs)
    return s  # Return anyway; let it fail with a clear error later
import numpy as np
...
class ErrorCalculator():
    def __init__(self,
        ...
        self.base_point = original_model['model'].base_point
        ...
        self.original_model.transfer_var = Var(initialize=0.0)
        self.original_model.min_distance_base_point = (
            Objective(expr=sum(self.original_model.direction[j] * (self.original_model.var_proj[j] - self.base_point[j])
                               for j in range(self.dim)),
                      sense=minimize))
        self.original_model.min_distance_base_point.deactivate()
        if is_epigraph:
            self.original_model.max_obj = Objective(expr = self.original_model.obj, sense = maximize)
            self.original_model.max_obj.deactivate()
        self.solver = _resolve_solver_factory(solver, tee=False)

        def add_constraints_rule(model, i):
            d_xi = model.var_proj[i] - self.base_point[i]
            d_vi = model.direction[i]
            if value(d_vi) == 0:
                return Constraint.Skip
            else:
                return d_xi == d_vi * self.original_model.transfer_var
        self.original_model.add_constraints = Constraint(range(self.dim), rule=add_constraints_rule)
        self.original_model.add_constraints.deactivate()
        ...
        self.approx_model.min_error = (
            Objective(expr=sum((self.approx_model.var_proj[j] - self.approx_model.x_org[j]) ** 2
                               for j in range(self.dim)),
                      sense=minimize))
        self.approx_model.transfer_var = Var(initialize=0.0)
        self.approx_model.min_distance_base_point = (
            Objective(expr=sum(self.approx_model.direction[j] * (self.approx_model.var_proj[j] - self.base_point[j])
                               for j in range(self.dim)),
                      sense=minimize))
        self.original_model.min_distance_base_point.deactivate()
        def add_constraints_rule_apx(model, i):
            d_xi = model.var_proj[i] - self.base_point[i]
            d_vi = model.direction[i]
            if value(d_vi) == 0:
                return Constraint.Skip
            else:
                return d_xi == d_vi * self.approx_model.transfer_var
        self.approx_model.add_constraints = Constraint(range(self.dim), rule=add_constraints_rule_apx)
        self.approx_model.add_constraints.deactivate()
        ...
        self.cvx_solver = _resolve_solver_factory('gurobi', tee=False)
    ...
    def _cal_max_obj(self):
        ...
        solver = _resolve_solver_factory(self.original_model_dict.get('fmax_solver',self.solver_str))
    ...
    def project(self, target: np.ndarray, to_approx: bool = False) -> Optional[np.ndarray]:
        if to_approx:
            ...
            self.approx_model.min_distance_base_point.deactivate()
    ...
    def optimize_direction(self, 
        ...
        if in_approx:
            ...
            self.approx_model.min_direction.deactivate()
            self.approx_model.min_distance_base_point.activate()
            self.approx_model.add_constraints.activate()
            for j in range(self.dim):
            ...
            self.original_model.min_error.deactivate()
            self.original_model.min_direction.deactivate()
            self.original_model.min_distance_base_point.activate()
            self.original_model.add_constraints.activate()
            for j in range(self.dim):
            ...
            try:
                ...
                if results.solver.termination_condition == TerminationCondition.optimal:
                    self.original_model.add_constraints.deactivate()
                    #print(('direction', in_approx, results.solver.termination_condition, direction))
                    #print(np.array([self.original_model.V_sq[j].value for j in range(33)]))
                    #print(np.array([self.original_model.var_proj[j].value for j in range(self.dim)]))
                    return np.array([self.original_model.var_proj[j].value for j in range(self.dim)])
                #return np.array([self.original_model.var_proj[j].value for j in range(self.dim)])
            except:
                print(self._iter)
        ...
        for i in range(n_cal):
            ...
            if cal_feas:
                ...
                if x_apx is not None and x_org is not None:
                    ...
                    self.cal_feas_error_analyze(c, in_approx=True)
                    print(self._iter)
            if cal_opt:
                # 最优性误差路径
    ...
    def cal_feas_error_analyze(self,direction,in_approx: bool = False):
        self.approx_model.min_error.deactivate()
        self.approx_model.min_direction.activate()
        self.approx_model.min_distance_base_point.deactivate()
        for j in range(self.dim):
        ...
        results = self.cvx_solver.solve(self.approx_model)
        if results.solver.termination_condition != TerminationCondition.optimal:
            x_apx_test = np.array([self.original_model.var_proj[j].value for j in range(self.dim)])
            residuals_test = self.A_hat @ x_apx_test - self.b_hat
            unbound_mark = np.zeros([self.A_hat.shape[0],1])
            for i in range(residuals_test.shape[0]):
                if residuals_test[i] > 0:
                    unbound_mark[i] = 1
        print(1)
...
class PreTrainNet(nn.Module):
    def __init__(self,A_init,b_init,is_epigraph = False,device = 'cpu'):
        ...
        if is_epigraph:
            ...
            for it in range(len(A_init)):
                self.nrows.append(A_init[it].shape[0])
                self.dim_x.append(A_init[it].shape[1])
                self.A_net.append(
                    nn.Sequential(ZeroInitLinear(0, self.nrows[it] * self.dim_x[it], init_bias=np.array(A_init[it]).flatten(), device=device))
                )
                
        self.total_row_num = sum([self.nrows[it] for it in range(len(self.nrows))])
        self.total_dim_x_num = sum([self.dim_x[it] for it in range(len(self.dim_x))])

        self.b_net = nn.Sequential(
            ZeroInitLinear(0, self.total_row_num, init_bias=np.array(b_init), device=device)
        )


    def forward(self):
        # 生成A矩阵
        A = torch.zeros(self.total_row_num, self.total_dim_x_num, device=self.device)
        for it in range(len(self.A_net)):
            A_flat = self.A_net[it](torch.empty(0, device=self.device))
            A_trans1 = A_flat.view(-1,self.nrows[it], self.dim_x[it])
            #empty_matrix = torch.tensor(np.zeros([self.nrows[it],self.total_dim_x_num - self.dim_x[it]]),dtype=torch.float32, device=A.device).reshape(-1,self.nrows[it],self.total_dim_x_num - self.dim_x[it])
            #A_trans2 = torch.cat([A_trans1, empty_matrix], dim=0)
            A[sum(self.nrows[i] for i in range(0,it)): sum(self.nrows[i] for i in range(0,it + 1)),
              sum(self.dim_x[j] for j in range(0,it)): sum(self.dim_x[j] for j in range(0,it + 1))] = A_trans1
        '''
        ...
        '''
        # 归一化A的行向量
        A = A.reshape(-1, self.total_row_num, self.total_dim_x_num)
        ...
        # 生成b向量
        b = self.b_net(torch.empty(0, device=self.device))  # shape: (batch_size, 2)
...
class Trainer:
    ...
    def initialize(self):
        ...
        if hasattr(self.model, 'A_net'):
            for it in range(len(self.model.A_net)):
                param_groups.append({'params': self.model.A_net[it].parameters(), 'lr': lr_A})
    ...
    def _train_serial(self, n_train: int = None, params_data = None):
        ...
        for epoch in range(n_train+1):
            for batch_data in params_data['dataloader']:
                ...
                weight_feas = 1
                weight_opt = 1
                if epoch < n_train * 0.6:
                    weight_feas = 1
                    weight_opt = 1
                elif epoch >= n_train * 0.6:
                    weight_feas = 1
                    weight_opt = 1e-2
                elif epoch >= n_train * 0.8:
                    weight_feas = 1
                    weight_opt = 1e-4
                loss_total = loss_feas * weight_feas + loss_opt * weight_opt
                ...
                #callback_iterator=[1,25,50,75,100,300,500,700,900,1000]
