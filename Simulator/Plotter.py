...
class ErrorVisualizer:
        ...
        for _ in range(num_sample):
            ...
            if x_org is not None:
                if error_feas==[]:
                    ...
                    error_feas.append(np.sum((x_apx - x_org) ** 2))
                a = np.sum((x_apx - x_org)  ** 2)
            # 计算最优性误差
            x_org1 = model.optimize_direction(c)
            if x_org1 is None:
                print(1)
            x_apx1 = model.project(x_org1, to_approx=True) if x_org is not None else None
            if x_apx1 is not None:
                #error_opt.append(np.sum((x_apx1 - x_org1)  ** 2) / 2 / 1000)
                error_opt.append(np.sum((x_apx1 - x_org1) ** 2))
                b = np.sum((x_apx1 - x_org1)  ** 2)
    ...
    def plot_dual_violin(self, save_path=None, figsize=(12, 8)):
        ...
        plt.legend(handles=legend_elements,
                   ...
                   fontsize=12,
                   framealpha=0.9,
                   ncol=len(legend_elements),  # 关键：每个元素一列
                   bbox_to_anchor=(0.5, 1.07))  # 将图例放在图形上方

        plt.subplots_adjust(top=0.85)  # 调整顶部空间，给图例留位置
    ...
    def plot_kde_evolution(self, save_path=None, figsize=(12, 10)):
        """
        修复版KDE可视化方案，输出三个独立的图片：
        1. Feasibility Error 分布演化
        2. Optimality Error 分布演化
        3. 迭代颜色标尺

        使用Times New Roman字体，字号26
        """
        # 设置全局字体为Times New Roman，字号26
        plt.rcParams['font.family'] = 'Times New Roman'
        plt.rcParams['font.size'] = 24
        plt.rcParams['axes.titlesize'] = 24
        plt.rcParams['axes.labelsize'] = 24
        plt.rcParams['xtick.labelsize'] = 20  # X轴刻度稍小一些
        plt.rcParams['ytick.labelsize'] = 20  # Y轴刻度稍小一些
        plt.rcParams['legend.fontsize'] = 20  # 图例字体稍小一些
        ...
        # =====================
        # 图片1：Feasibility Error
        # =====================
        fig1 = plt.figure(figsize=(figsize[0], figsize[1] * 0.45))  # 使用原高度的一部分
        ...
        ax1.set_xlabel('Feasibility Error Value', fontname='Times New Roman', fontsize=24)
        ax1.set_ylabel('Probability Density', fontname='Times New Roman', fontsize=24)
        ...
        # 保存第一个图片
        if save_path:
            # 如果是文件路径，自动生成第一个图片的保存路径
            if isinstance(save_path, str):
                name, ext = os.path.splitext(save_path)
                save_path1 = f"{name}_feasibility{ext}"
                save_path1_pdf = f"{name}_feasibility.pdf"
                fig1.savefig(save_path1, dpi=300, bbox_inches='tight')
                fig1.savefig(save_path1_pdf, bbox_inches='tight')
        # plt.show()
        plt.close(fig1)

        # =====================
        # 图片2：Optimality Error
        # =====================
        fig2 = plt.figure(figsize=(figsize[0], figsize[1] * 0.45))  # 使用原高度的一部分
        ...
        ax2.set_xlabel('Optimality Error Value', fontname='Times New Roman', fontsize=24)
        ax2.set_ylabel('Probability Density', fontname='Times New Roman', fontsize=24)
        ...
        # 保存第二个图片
        if save_path:
            # 如果是文件路径，自动生成第二个图片的保存路径
            if isinstance(save_path, str):
                name, ext = os.path.splitext(save_path)
                save_path2 = f"{name}_optimality{ext}"
                save_path2_pdf = f"{name}_optimality.pdf"
                fig2.savefig(save_path2, dpi=300, bbox_inches='tight')
                fig2.savefig(save_path2_pdf, bbox_inches='tight')
        # plt.show()
        plt.close(fig2)
        ...
        # 图片3：颜色标尺
        # =====================
        fig3 = plt.figure(figsize=(figsize[0], 0.8))  # 窄图用于颜色条

        # 创建颜色条
        sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)

        # 设置标尺刻度为整数迭代
        tick_positions = np.linspace(0, max_iter, num=max_iter + 1)

        # 添加颜色条
        ax3 = fig3.add_subplot(111)
        cbar = plt.colorbar(sm, cax=ax3, orientation='horizontal')
        cbar.set_label('Iteration Progress', fontname='Times New Roman', fontsize=24)
        ...
        for pos in tick_positions:
            if pos != 0:
                tick_labels.append(f"{int(iterations[int(pos)] - 1)}")
            else:
                tick_labels.append(f"{int(0)}")
        ...
        # 保存第三个图片
        if save_path:
            # 如果是文件路径，自动生成第三个图片的保存路径
            if isinstance(save_path, str):
                name, ext = os.path.splitext(save_path)
                save_path3 = f"{name}_colorbar{ext}"
                save_path3_pdf = f"{name}_colorbar.pdf"
                fig3.savefig(save_path3, dpi=300, bbox_inches='tight')
                fig3.savefig(save_path3_pdf, bbox_inches='tight')
        # plt.show()
'''
    def plot_kde_evolution(self, save_path=None, figsize=(8, 10)):
        ...
        # 共享的配置参数
        delta_threshold = 1e-5  # 用于判断是否绘制冲激函数的阈值
        spike_marker = '^'  # 冲激函数标记样式
        spike_height_factor = 0.9  # 冲激函数高度比例
        ...
        ax2.set_ylim(0, global_y_max_opt)  # 统一设置y轴范围
        #ax2.set_xlabel('Optimality Error Value'+ r'$(\times 10^{-3})$')
        ...
        for pos in tick_positions:
            if pos != 0:
                ...
                tick_labels.append(f"{int(0)}")
        #tick_labels = [f"{int(iterations[int(pos)] - 1)}"
                       #for pos in tick_positions]
