...
import matplotlib
from scipy.ndimage import gaussian_filter1d  # 添加高斯滤波器
...
def savgol_filter(data, window_length=51, polyorder=3):
    """Savitzky-Golay滤波器"""
    from scipy.signal import savgol_filter
    return savgol_filter(data, window_length, polyorder)


def smooth_data(data, method='ema', **kwargs):
    """
    数据平滑函数

    参数:
    data: 输入数据数组
    method: 平滑方法 ('ma'=移动平均, 'ema'=指数移动平均, 'gaussian'=高斯滤波, 'savgol'=Savitzky-Golay)
    **kwargs: 平滑参数
...
def apply_smoothing_to_errors(feasibility_errors, optimality_errors,
                              method='gaussian', **smoothing_kwargs):
    """
    对可行性误差和最优性误差应用平滑
    ...
    for i in range(len(feasibility_errors)):
        # 平滑可行性误差
        feas_smoothed = smooth_data(feasibility_errors[i], method=method, **smoothing_kwargs)
        smoothed_feas.append(feas_smoothed)

        # 平滑最优性误差
        opt_smoothed = smooth_data(optimality_errors[i], method=method, **smoothing_kwargs)
        smoothed_opt.append(opt_smoothed)

    return smoothed_feas, smoothed_opt
